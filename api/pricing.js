// api/pricing.js — pricing management
// POST { action:'chat', message, pricingData, history } → AI assistant (was api/pricing-chat.js)
// POST { action:'save', library, templates }            → persist to GitHub (was api/save-pricing.js)
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        fixed_cost_refs:    { type: 'array',  items: { type: 'string' }, description: 'Library IDs for shared group costs' },
...
async function handleSave(req, res) {
  ...
  const apiBase = 'https://api.github.com/repos/ubuntubaliyoga/ubuntu/contents/data/pricing.json';
  const headers = {
  ...
  const content = Buffer.from(JSON.stringify({ library, templates }, null, 2) + '\n').toString('base64');
  const putRes = await fetch(apiBase, {
    method: 'PUT', headers,
    body: JSON.stringify({ message: 'chore: update pricing data via admin UI', content, branch: 'main', ...(sha ? { sha } : {}) }),
  });
  if (!putRes.ok) {

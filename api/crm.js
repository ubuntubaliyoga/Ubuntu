...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'update') {
      ...
      if (status !== undefined) {
        if (db === 'leads') props['Instagram'] = { select: { name: status } };
        else props['Status'] = { multi_select: status.map(s => ({ name: s })) };
      }
    ...
    if (action === 'create') {
      const { name, company, email, insta, location, notes } = req.body;
